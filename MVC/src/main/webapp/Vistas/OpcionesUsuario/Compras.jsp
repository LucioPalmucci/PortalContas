                    ...
                    <table class="table table-hover align-middle mb-0 filas-espaciadas tabla-tarjetas-movil">
                        ...
                        <c:forEach var="compra" items="${compras}">
                            <tr data-fila
                                ...
                                <c:if test="${esAdmin}"><td data-label="Cliente" class="col-extra">${compra.usuario.nombreCompleto}</td></c:if>
                                <td data-label="Fecha"><fmt:formatDate value="${compra.fecha}" pattern="dd/MM/yyyy"/></td>
                                <td data-label="Concepto" class="fw-semibold">${compra.conceptoCompra}</td>
                                <td data-label="Precio unit." class="col-extra"><fmt:formatNumber value="${compra.precioUnitario}" type="currency" currencySymbol="$"/></td>
                                <td data-label="Cant." class="col-extra">${compra.cantidad}</td>
                                <td data-label="Total" class="fw-semibold"><fmt:formatNumber value="${compra.valorTotal}" type="currency" currencySymbol="$"/></td>
                                <td data-label="Medio" class="col-extra">${compra.metodo.nombre}</td>
